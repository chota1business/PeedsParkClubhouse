...
-- Item #2 (2026-09-03 batch): overnight blocks (e.g. Pool 5pm-7am next day)
-- were not reflecting correctly in slot availability.
--
-- Root cause: blocks.start_at/end_at are `timestamptz` (correctly stored in
-- UTC). get_facility_slots() built comparison timestamps as `p_date + time`
-- (a plain, zone-less timestamp) and let Postgres implicitly cast that to
-- timestamptz using the DB session's timezone, which is UTC — not the
-- Asia/Kolkata wall-clock time the slot labels (and open_time/close_time)
-- actually represent. That silently shifted every comparison by +5:30,
-- so a block entered for 5pm-7am IST was being checked as if it were
-- 5pm-7am UTC, landing on the wrong hours (or missing the next day
-- entirely). Fix: explicitly interpret p_date + time as Asia/Kolkata wall
-- clock via `AT TIME ZONE 'Asia/Kolkata'` before comparing to the
-- timestamptz columns. Applies to both the hall/lawn full-day check and
-- the pool/badminton hourly loop.
      ...
      select exists (
        select 1 from blocks
         where facility_id = p_facility_id
           and start_at < ((p_date + time '23:59:59') at time zone 'Asia/Kolkata')
           and end_at > (p_date::timestamp at time zone 'Asia/Kolkata')
      ) into v_blocked;
    ...
    begin
      ...
      while h < extract(hour from v_close)::int loop
        ...
        select exists (
          select 1 from blocks
           where facility_id = p_facility_id
             and start_at < ((p_date + slot_end) at time zone 'Asia/Kolkata')
             and end_at > ((p_date + slot_start) at time zone 'Asia/Kolkata')
        ) into v_blocked;
