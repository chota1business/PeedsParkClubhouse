...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  AdminActions.init({ staff, facilityIds: () => Object.keys(FACILITY_LABELS), onSaved: loadFeed, expenses: false });
  wireAddEnquiryModal();
...
const FEED_STATUS_OPTIONS = {
  pending: [["pending", "Pending"], ["approved", "Confirm Booking"], ["cancelled", "Cancelled"]],
...
// starts as "new").
async function openEnquiryModal(row) {
  const modal = document.getElementById("enquiryModal");
  const form = document.getElementById("enquiryForm2");
  form.reset();
  const statusWrap = document.getElementById("enquiryStatusFieldWrap");
  if (row) {
    const { data: full, error } = await supabaseClient.from("enquiries").select("*").eq("id", row.id).single();
    if (error) { alert("Could not load this enquiry: " + error.message); return; }
    row = { ...row, ...full, activity_date: full.preferred_date, notes: full.message };
    form.elements["guests"].value = row.guests || "";
  }
  ...
  AdminActions.prepareEnquiry(row);
  modal.hidden = false;
}
