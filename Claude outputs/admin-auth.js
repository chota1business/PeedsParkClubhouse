...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (forgotForm) setupForgotPasswordLink(forgotForm);
...
// "Forgot password?" link — reveals the forgotForm (hidden by default in
// index.html) and, on submit, emails a recovery link via Supabase Auth that
// lands on reset-password.html. This was previously unwired entirely: the
// link and form existed in the HTML but nothing opened the form or handled
// its submit.
function setupForgotPasswordLink(forgotForm) {
  ...
  if (link) {
    link.addEventListener("click", (e) => {
      ...
      link.closest("p")?.setAttribute("hidden", "");
  ...
  forgotForm.addEventListener("submit", async (e) => {
    ...
    if (messageEl) {
      // Deliberately the same message whether or not the email is a real
      // staff account — Supabase itself doesn't reveal that either way, and
      // this avoids letting the login page be used to probe which emails
      // have staff accounts.
